...
describe('isSupportedOperator', () => {
    describe('지원되는 연산자라면, true를 반환한다.', () => {
        it.each(['+', '-', '*', '/'])(
            'isSupportedOperator(%p) = true',
            (op) => {
                expect(isSupportedOperator(op)).toBe(true);
            },
        );
    });
    describe('지원되지 않는 연산자라면, false를 반환한다.', () => {
        it.each(['%', '**', '^', 'a'])(
            'isSupportedOperator(%p) = false',
            (op) => {
...
describe('operate', () => {
    ...
    describe('두 개의 피연산자에 대해 나눗셈 결과를 반환한다.', () => {
        describe('피연산자 중 0이 있는 경우, 올바른 나눗셈 결과를 반환한다.', () => {
            ...
            describe('두 피연산자가 모두 0이 아닌 경우, 올바른 나눗셈 결과를 반환한다.', () => {
                it.each([
                    { operand1: 1, operand2: 30 },
                    { operand1: -1, operand2: 30 },
                    { operand1: 1, operand2: -30 },
